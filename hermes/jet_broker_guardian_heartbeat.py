#!/usr/bin/env python3
"""Publish a sanitized Jet Broker synthetic journey heartbeat."""
...
GUARDIAN_ISSUE = "10"
PUBLIC_HEALTH = "https://broker.getjetai.com/health"
GATEWAY = "http://127.0.0.1:18765"
EXPECTED_MODEL = "brokercompliance"
...
def build_payload(token: str, *, now: str | None = None) -> dict:
    ...
    try:
        app = request_json(PUBLIC_HEALTH)
        result["app_healthy"] = (
            isinstance(app, dict)
            and app.get("status") == "ok"
            and app.get("deployment_locked") is False
            and isinstance(app.get("source_record_count"), int)
            and app["source_record_count"] > 0
        )
    except Exception:
        pass
    if not token:
