...
def test_public_status_shell_reads_sanitized_guardian_state_without_html_injection():
    ...
    html = page.read_text()
    assert "raw.githubusercontent.com/Eden-TDG/agent-tech-guardian/guardian-state/status.json" in html
    assert "api.github.com/repos/Eden-TDG/agent-tech-guardian/issues/1" not in html
